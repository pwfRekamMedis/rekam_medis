                ...
                <div class="col-md-12">
                    <div class="card">
						...
						{{ csrf_field() }}
							...
							@foreach ($pasien as $p)<option value="{{ $p -> id_pasien }}">{{$p -> nama_pasien}}</option>@endforeach</select></br>
							</div>
							
							<div class="form-group">
							<label>Nama Petugas</label>
                            <select name="id_petugas" required="required" class="form-control" placeholder=". . .">
							@foreach ($petugas_admin as $pa)<option value="{{ $pa -> id_petugas }}">{{$pa -> nama_petugas}}</option>@endforeach</select></br>
							</div>
							
							<div class="form-group">
							<label>Nama Dokter</label>
                            <select name="id_dokter" required="required" class="form-control" placeholder=". . ."> 
							@foreach ($dokter as $d)<option value="{{ $d -> id_dokter }}">{{$d -> nama_dokter}}</option>@endforeach</select></br>
							</div>
